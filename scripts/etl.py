# -*- coding: utf-8 -*-
"""
Created on Sat Apr 10 12:50:57 2021
...
##################
    #all settings for this script are contained in the config.yaml file. load them for use within this function.
    ...
    training_files_path = config['etl']['training_files_path']
    processed_path = Path(config['etl']['processed_path'])
    test_files_list = config['etl']['test_files_list']
    test_files_path = Path(config['etl']['test_files_path'])
    ...
    ##################
    #there is a csv file which contains the filenames of the training examples, and the times at which the applicable section appears. 
    #Load that file to a Pandas dataframe
    train_tp = pd.read_csv(raw_data_file)
    #if we are in development mode, shorten the dataframe so this doesn't take a million years
    if dev_mode == True:
      train_tp = train_tp[1:100]
    #there is also a csv file that lists all the files we need to test. load into a pandas dataframe
    ...
    """
    this function loops through the training examples in train_tp (or any other pandas dataframe), converts them into mel spectrograms, 
    and saves them, along with label data and row id data as a pickle file. If this is being run with reprocess_audio==False, it just loads the old pickle files rather than recreate them.
    This is a function, rather than a single for loop so it can be called multiple times on the same set of data to allow for data augmentation
    This is defined as an inner function so it can share variables with the etl function.
    """
    def process_a_set_of_training_data(
                              ...
                              #the keys from the dictionary above, in the order lables, specs, rowids
        ...
        if reprocess_audio==True:          
            ...
            for row_num in range(0, len(input)):
                #for each training example, get the audio file name, and the start and end times of the section of interest
                ...
                outputs[output_lsr_keys[1]].append(get_mel_spectrogram(row_num=row_num, start=section_start, end=section_end, filename = section_contained_in_filename,\
                                     path=training_files_path, sr=SR, length=LENGTH_OF_SECTION, frame_size=FRAME_SIZE, hop_size=HOP_SIZE,\
                                     mode='Train', test_clip_num=0, addnoise=0, timeshift=0, pitchfactor=0, speedfactor=0))
                #add row ids to list
        ...
        return outputs[output_lsr_keys[0]], outputs[output_lsr_keys[1]], outputs[output_lsr_keys[2]]


    #run the process_a_set_of_training_data function at least once
    labels, spectrograms, row_ids = process_a_set_of_training_data(reprocess_audio=reprocess_audio, output_lsr_keys=['labels', 'spectrograms', 'row_ids'])
    
    #if we want to do any augmentation, run it for each of the 4 optional augmentations as well
    if aug_1_YN==True:
        labels_aug1, spectrograms_aug1, row_ids_aug1 = process_a_set_of_training_data(reprocess_audio=reprocess_audio, output_lsr_keys=['labels_aug1', 'spectrograms_aug1', 'row_ids_aug1'], addnoise=aug_1_addnoise, timeshift=aug_1_timeshift, pitchfactor=aug_1_pitchfactor, speedfactor=aug_1_speedfactor)

    if aug_2_YN==True:
        labels_aug2, spectrograms_aug2, row_ids_aug2 = process_a_set_of_training_data(reprocess_audio=reprocess_audio, output_lsr_keys=['labels_aug2', 'spectrograms_aug2', 'row_ids_aug2'], addnoise=aug_2_addnoise, timeshift=aug_2_timeshift, pitchfactor=aug_2_pitchfactor, speedfactor=aug_2_speedfactor)
    
    if aug_3_YN==True:
        labels_aug3, spectrograms_aug3, row_ids_aug3 = process_a_set_of_training_data(reprocess_audio=reprocess_audio, output_lsr_keys=['labels_aug3', 'spectrograms_aug3', 'row_ids_aug3'], addnoise=aug_3_addnoise, timeshift=aug_3_timeshift, pitchfactor=aug_3_pitchfactor, speedfactor=aug_3_speedfactor)
    
    if aug_4_YN==True:
        labels_aug4, spectrograms_aug4, row_ids_aug4 = process_a_set_of_training_data(reprocess_audio=reprocess_audio, output_lsr_keys=['labels_aug4', 'spectrograms_aug4', 'row_ids_aug4'], addnoise=aug_4_addnoise, timeshift=aug_4_timeshift, pitchfactor=aug_4_pitchfactor, speedfactor=aug_4_speedfactor)

    #['labels_aug1', 'labels_aug2', 'labels_aug3', 'labels_aug4']
    #['spectrograms_aug1', 'spectrograms_aug2', 'spectrograms_aug3', 'spectrograms_aug4']
    #['labels_aug1', 'labels_aug2', 'labels_aug3', 'labels_aug4']


    ##################
    # Tranform Final Predictions Data used for Submission
    ##################


    #when using the word test in this data, I am talking about the final test examples we run with our model. Not the test set used in training.

    #for the test files, we need to know how to split them up into segments that match the training data in length. 
    #Should the segments overlap or not?  
    ...
    #         labels = pickle.load(f)
        ...
        for i in range(0, len(test_files)):
            ...
            for j in range(0, len(test_segments)):
                ...
                #create a spectrogram and add to the list
                spectrograms_test.append(get_mel_spectrogram(row_num=i, start=0, end=0, filename=test_file_name, path=test_files_path,\
                sr=SR, length=LENGTH_OF_SECTION, frame_size=FRAME_SIZE, hop_size=HOP_SIZE, mode="Test", test_clip_num=j, addnoise=0, timeshift=0, pitchfactor=0, speedfactor=0, test_segments=test_segments))
    ...
    ##################
    #leaving this section in as a placeholder. the exporting has already been taken care of in pieces, due to the size of the data.
    #moving train test split into the training script so it could be done on the fly and there is no need to save the files to disk
    
    # train test split
    #logger.info("-------------------Train test split & Export-------------------")
    #train, test = train_test_split(train_tp, test_size=test_size, random_state=random_state)
    # export data
    ...
    if train_test_split_method == 'Simple':
        #this just creates a train test split on the unaugmented data. The augmented data is not used.
        train_rowids, test_rowids = custom_train_test_splitter(method='Simple', random_state=random_state, test_size=test_size, row_ids_list=row_ids)
...
##################
# Define Functions used in transformation
##################
...
In the training set, each file one or more "sections" that are of interest. Each of these is annotated by a single row 
in the train_tp, or train_fp dataframes.
This function gets the audio for a given section, based on the row number from the dataframe

"""


def get_flac_section(row_num, start, end, filename, path, sr, length, addnoise=0, timeshift=0, pitchfactor=0, speedfactor=0):
    #this function is called by the get_mel_spectrogram function while processing training data
        
    #start = float(train_tp.iloc[row_num]['t_min'])
    #end = float(train_tp.iloc[row_num]['t_max'])
    #filename = train_tp.iloc[row_num]['recording_id']
        
    #find the midpoint of the training section as defined in the data provided, 
    #and then re-assign the start and end variables to create a section of length 'length', starting length/2 before the midpoint 
...
def get_mel_spectrogram(row_num, start, end, filename, path, sr, length, frame_size, hop_size, mode='Train', \
                        test_clip_num=0, addnoise=0, timeshift=0, pitchfactor=0, speedfactor=0, test_segments=[]):
    
    if mode=='Train':
        y, sr = get_flac_section(row_num=row_num, start=start, end=end, filename=filename, path=path, sr=sr, length=length, addnoise=addnoise, timeshift=timeshift, pitchfactor=pitchfactor, speedfactor=speedfactor)
    if mode=='Test':
        y, sr = get_flac_section_for_test(row_num=row_num, filename=filename, which_segment=test_clip_num, path=path, sr=sr, test_segments=test_segments)
    spectrogram = librosa.feature.melspectrogram(y=y, sr=int(sr), n_fft=int(frame_size), hop_length=int(hop_size))
    spectrogram = librosa.power_to_db(spectrogram, ref=np.max) # Converting to decibals
    
    #resize. Not sure why we would do this. maybe to reduce the load on the nn? any reason we would choose these dimensions?
    spectrogram = resize(spectrogram, (224, 400))
    #spectrogram = resize(spectrogram, (128, 660))
    
    # Normalize to 0...1 - this is what goes into neural net
    # Q) when you normalize, mel_spec = mel_spec / np.max(mel_spec), should we divide by max - min instead?
    # A) Negative numbers are taken care of in previous line (with mel_spec overwriting), 
    # so by this line it is already 0…some number spectrogram. Proper (and slightly more complicated) way to do it is shown at 
    #https://pytorch.org/hub/pytorch_vision_resnet/ - but this 2-liner is easy to understand and works well.
...
def get_test_segments(overlap, sr):
    ...
    if overlap==True:
        test_segments['start'] = [0, 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54]
        test_segments['end'] = [6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60]
    else:
